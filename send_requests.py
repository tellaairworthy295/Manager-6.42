...
# Configuration
API_URL = "http://localhost:5005/news/scrape_news"
search_query = 'site:bloomberg.com/news/articles China OR "Hong Kong" OR AI'
#search_query = 'site:reuters.com/world/china'
def get_rate_limit():
    """Returns 4 for weekends, 3 for weekdays."""
    today = datetime.now()
    return 4 if today.weekday() >= 5 else 3
...
def get_wait_time():
    ...
    - High Freq Night: 20:30 to 07:30
    - Medium Freq Day: 07:30 to 20:30
    """
    now = datetime.now()
    weekday = now.weekday()
    ...
    if is_weekend:
        base_min, base_max = 15, 30  # 24-36 mins
    else:
        base_min, base_max = 7, 14   # 6-14 mins
    ...
    if is_daytime:
        ...
        if is_weekend:
            multiplier = 1.5 
            period_desc = "Weekend Day (07:30-20:30)"
        else:
            multiplier = 1.6
            period_desc = "Weekday Day (07:30-20:30)"
        
        final_wait_time = int(round(base_wait_seconds * multiplier))
    else:
